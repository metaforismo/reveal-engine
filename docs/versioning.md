...
| Snapshot         | module-owned, current book schemas are v2 | reject unknown versions                             |
...
The same no-invention rule retires progressive `round-book-v1`,
`cards-book-v1`, and `staged-survival/book-v1`: none records the commitment
that had to be published before play, so converting one to v2 would fabricate
the fact the new schema exists to prove. Their committed files remain negative
fixtures. AETHER's v1 ticket family is likewise refused for new settlement
because it did not bind adapter economics; the original bytes remain pinned as
historical evidence and v2 fixtures cover the current protocol.
