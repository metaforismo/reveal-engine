...
| Reconnect state or receipt tampering                                 | definition binding, step replay, receipt/accounting/cap validation, one idempotency key per receipt; production still needs authenticated storage                                                                                                                                                                  |
...
| Consistent whole-round rewrite of a stored snapshot                  | **not detectable in process** — command fingerprints are unkeyed hashes over public fields, so a rewritten snapshot of another round reconciles perfectly. `PermutationBook.restore()` requires the published round for any bound snapshot, and that value must be read from a store independent of the snapshot's |
