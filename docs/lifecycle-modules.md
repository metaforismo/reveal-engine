...
All four ship today: `progressive-market`,
`sequential-cards` ([`modules/sequential-cards.md`](modules/sequential-cards.md)),
`staged-survival` ([`modules/staged-survival.md`](modules/staged-survival.md))
and `permutation` ([`modules/permutation.md`](modules/permutation.md)). None of
them required a core change, which is the load-bearing fact about this table:
the contract was designed against four shapes and all four landed inside it.
...
**Reconnect evidence is now part of the contract.** Every lifecycle shape
declares `roundBinding`, and
`book.restore(definition, snapshot, expectedBinding)` requires the independently
published round. A bound snapshot never authorizes itself: `null` is accepted
only for a genuinely unbound snapshot, and omission is a typed
`COMMITMENT_MISMATCH`. This widened the slot when the condition this document
previously deferred on arrived — progressive-market, sequential-cards and
staged-survival all needed the same out-of-band fact as permutation. ADR 0011
records why the binding cannot live inside the re-sealable snapshot.
