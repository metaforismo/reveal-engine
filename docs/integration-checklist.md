...
- [ ] For the permutation module, the published round passed to `PermutationBook.restore(definition, snapshot, publishedBinding)` is read from the **published-round record, not from the snapshot store**. `restore()` requires the argument for any bound snapshot and refuses without it, but a host that reads both from one store has re-pointed nothing and defended nothing: a whole-round rewrite of a snapshot is consistent, cheap and undetectable in process (`docs/modules/permutation.md` §9.1), and this argument is the only control against it. Reads from the two must be independently authenticated.
