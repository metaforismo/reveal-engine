...
import type { EvidenceEvent, GameDefinition, Posterior, RoundContext } from './contracts.js';

export { assertBoundedBigInt, assertRational };
...
export function assertContext(value: unknown, path = '$'): asserts value is RoundContext {
  ...
  assertIdentifier(value.gameId, `${path}.gameId`);
...
export function assertEvidenceEvent(
  ...
  assertIdentifier(value.label, `${path}.label`, 'INVALID_ADAPTER', ENGINE_LIMITS.maxLabelBytes);
}
...
export function assertGameDefinition(value: unknown, path = '$'): asserts value is GameDefinition {
  ...
  assertIdentifier(value.adapterVersion, `${path}.adapterVersion`);
  ...
  value.outcomes.forEach((outcome, index) => {
    assertIdentifier(outcome, `${path}.outcomes[${index}]`);
    if (ids.has(outcome))
  ...
  assertIdentifier(value.evidence.modelVersion, `${path}.evidence.modelVersion`);
